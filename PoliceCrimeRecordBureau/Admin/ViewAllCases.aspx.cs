...
{
    public partial class ViewAllCases : System.Web.UI.Page
    {
        ...
        protected void DropDownList2_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (DropDownList2.SelectedIndex != 0)
            {
                ...
                string did = db.extscalr("select District_id from districtAdmin where username='" + Session["user"] + "'");
                ds = db.discont("select * from File_Info where category='" + DropDownList2.SelectedValue + "'");
                if (ds.Tables[0].Rows.Count > 0)
                {
                    DataList1.Visible = false; DataList2.Visible = true;
        ...
        {
            Response.Redirect("ViewAllCases.aspx");
        }
        ...
        protected void DropDownList3_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if ((DropDownList3.SelectedIndex != 0)&&(DropDownList2.SelectedIndex!=0))
            {
                ...
                string did = db.extscalr("select District_id from districtAdmin where username='" + Session["user"] + "'");
                ds = db.discont("select * from File_Info where isActive='" + DropDownList3.SelectedValue + "' and category='"+DropDownList2.SelectedValue+"'");
                if (ds.Tables[0].Rows.Count > 0)
                {
                    DataList1.Visible = false; DataList2.Visible = true;
                    DataList2.DataSource = ds;
